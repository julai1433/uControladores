...
int main(void)
{
  ...
  MX_TIM1_Init();
	...
	if(HAL_I2C_IsDeviceReady(&hi2c1,i,1,10)==HAL_OK){HAL_GPIO_TogglePin(GPIOC,GPIO_PIN_13);}

  while (1)
  {
		GetAngle(Angulo);								//Checa ángulo del MPU 6050
		if(Angulo[0]<0){								//Ángulo negativo, se especifíca la dirección de los motores hacia la izquierda
		...
		HAL_ADC_Start_IT(&hadc1);				//Checa Calibración de la constante
		//HAL_ADC_Start_IT(&hadc2);			//Checa Calibración de la constante
		controlPID();										//Evalua el ángulo y genera las señales P I y D.
		if(Angulo[0]>45){PID=0;					//Si el ángulo es mayor a 50, o menor que -50, apaga el motor.
		}else if(Angulo[0]<-45){PID=0;
		}else{
			PID=pid_p+pid_i+pid_d;}
		TIM1->CCR1 = PID;								//PID -> Salida PWM a los motores
		errorPrev=error;
		//SendData();										//Mandam Ángulo, Calibración y Señal de control PID por puerto serial.
  }
}
...
static void SendData (void){
	...
	sprintf(dataOut,"Angulo: %f	Kp: %f	Kd: %f	PID: %f\n", Angulo[0], kp, kd, PID);
...
void controlPID (void){
		kp=11.836734693877551020408163265306;//0.04081632653061224489795918367347*(ADC_Data);//9.3469387755102040816326530612246;
		kd=0.04081632653061224489795918367347*(ADC_Data);//49.224489795918367346938775510205;//
		ki=7.0081632653061224489795918367228;//0.004081632653061224489795918367347*(ADC_Data);//11.489795918367346938775510204082;//
		error=Angulo[0];
			pid_p = error*kp;										//Kp
			if(-4.0 < error < 4.0){							//Ki
			...
			pid_d = kd*((error-errorPrev)/dt);	//Kd
}
